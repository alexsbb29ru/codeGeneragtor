...
const generateFilesSpinner = $("#generateFilesSpinner")
...
inputText.on("keydown", function (e) {
	if (e.ctrlKey && e.key == "Enter") genQrButton.trigger("click")
})
...
window.onclick = (e) => {
	if (!e.target.attributes["histListFlag"] && historyAlert.is(":visible")) hideHistoryAlert()
}
...
function loadFontToBarcode() {
	barcode.loadFont(
		...
		100,
		fs.readFileSync(
			require("path").resolve(__dirname, "..") + "/fonts/PTSans-Regular.ttf",
			"binary"
		)
	)
}
...
function saveImageToBuffer() {
	if (!settingsBlock.general.copyImageToClipboard.isCopy) return false
...
async function getSettingsFromStorage(filename) {
	await getDataFromStorage(filename, function (data) {
		...
		if (!data) data = JSON.stringify(settingsBlock)
...
async function getHistoryFromStorage(filename, openDialog) {
	await getDataFromStorage(filename, function (data) {
		...
		if (!data) data = JSON.stringify(historyCodes)
		historyCodes = JSON.parse(data)

		if (openDialog) ipcRenderer.send("window:open-history", historyCodes)
	})
}
...
function getSavedCodesFromStorage(fileName) {
	getDataFromStorage(fileName, function (data) {
		if (!data) data = JSON.stringify(savedCodes)
...
saveButton.on("click", () => {
	if (qrImg.src) {
		...
		$("#fileNameInput")
			.val(removeCharacters(inputText.val()))
			.trigger("focus")
			.trigger("select")
	}
})
...
ipcRenderer.on("window:set-history", (event, historyText) => {
	...
	if (historyText.length > maxLength) historyText = historyText.substring(0, maxLength)
...
function writeRemainingSymb() {
	let remainingSymbCount =
		settingsBlock.general.codeSymbolLength.currentLength - inputText.val().length
...
function saveFileFunc(fileName, url) {
	...
	let result = fs.writeFileSync(pathToSave, base64Data, "base64")
	return result
}
 ...
 */
async function generateBarcode(text, callback) {
	...
	if (Array.isArray(text)) {
		...
		if (generateFilesSpinner.is(":visible")) generateFilesSpinner.hide()
		multiGenModal.modal("hide")
	} else {
		params.text = text

		await generateSingleCode(false, params)
	}

	if (callback) callback()
}

async function generateSingleCode(isMulti, params) {
	let promise = new Promise((resolve, reject) => {
		barcode.toBuffer(params, (err, png) => {
			if (err) {
				...
				if (isMulti) {
					let fileName = removeCharacters(params.text) + ".png"
					saveFileFunc(fileName, imageSrc)
					resolve(true)
				} else {
					qrImg.src = imageSrc
					...
					if (params.bcid == barcodeTypes.code128.bcid) {
						qrImg.height = 150
